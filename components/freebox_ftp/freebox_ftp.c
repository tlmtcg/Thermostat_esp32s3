...
// CORRECTION ALLOCATION HYBRIDE : Utilitaire pour sécuriser toutes les allocations du module
static void *safe_malloc(size_t size) {
    // Essai 1 : PSRAM
    void *ptr = heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    // Essai 2 : Fallback automatique en RAM interne (évite l'erreur 500 si la PSRAM sature)
    if (!ptr) {
        ptr = malloc(size);
    }
    return ptr;
...
{
    memset(res, 0, max_size);
    // CORRECTION : max_size - 1 pour s'assurer que le '\0' ne déborde jamais
    int bytes = recv(sock, res, max_size - 1, 0);
    if (bytes > 0)
    {
        res[bytes] = '\0';
...
static int send_command(int sock, const char *cmd, const char *arg, char *res, size_t res_size)
{
    // CORRECTION : Utilisation de safe_malloc pour éviter les plantages PSRAM
    char *buf = safe_malloc(BUF_SIZE);
    if (!buf)
    {
        ESP_LOGE(TAG, "Impossible d'allouer la mémoire pour la commande FTP");
        return -1;
    }
    ...
    ESP_LOGI(TAG, "> %.*s", len - 2, buf);
    int ret = send(sock, buf, len, 0);
    free(buf); // Le free est bien placé ici
...
static int ftp_connect_and_auth(char *res_buf, struct timeval *tv)
{
    ...
    ESP_LOGI(TAG, "Connexion au serveur %s:%d...", g_config.host, g_config.port);
    // CORRECTION : Taille exacte de la structure passée au socket
...
esp_err_t freebox_ftp_list(char *buffer, size_t buffer_size)
{
    char *res = safe_malloc(BUF_SIZE);
    if (!res)
    {
        ESP_LOGE(TAG, "Mémoire insuffisante pour 'res' (LIST)");
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t freebox_ftp_edit(const char *filename, void (*edit_callback)(char *buffer, size_t *len))
{
    ...
    size_t buffer_size = 32768; // 32 KB
    // CORRECTION : Allocation résiliente (PSRAM avec fallback RAM interne)
    char *work_buffer = heap_caps_calloc(1, buffer_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!work_buffer) {
        work_buffer = calloc(1, buffer_size);
    }
    
    if (!work_buffer)
    {
        ESP_LOGE(TAG, "Mémoire insuffisante pour le buffer d'édition");
        return ESP_ERR_NO_MEM;
    }
    ...
    if (err != ESP_OK)
    {
        if (bytes_read == 0) 
        {
            ESP_LOGW(TAG, "Fichier absent, création initiale.");
            err = ESP_OK;
        }
        else 
        {
            free(work_buffer); // Le free intermédiaire est correct ici
            return err;
        }
    }
    ...
    free(work_buffer); // Libération finale du gros buffer OK
    return err;
}
