...
char *heating_get_json(void)
{
    ...
    // Génération de la chaîne non-formatée
    char *json = cJSON_PrintUnformatted(root);
    
    // Nettoyage de l'arbre mémoire cJSON global
    cJSON_Delete(root);
    return json;

fail:
    ESP_LOGE(TAG, "Erreur mémoire lors de la construction du cJSON");
    cJSON_Delete(root);
    return NULL;
}
...
float heating_get_temp_current()
{
    ...
    // CORRECTION : Gestion propre et sécurisée de l'index du jour (Dimanche t.tm_wday = 0)
    int j = (t.tm_wday + 6) % 7; 
