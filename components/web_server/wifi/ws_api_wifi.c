...
static esp_err_t wifi_config_get_handler(httpd_req_t *req)
{
    ...
    cJSON *root = cJSON_CreateObject();
    if (!root) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM Root");
    }
    ...
    char *json = cJSON_PrintUnformatted(root);
    cJSON_Delete(root); // Libère l'arbre cJSON immédiatement après impression

    if (!json) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM Json");
    }

    httpd_resp_set_type(req, "application/json");
    esp_err_t res = httpd_resp_send(req, json, HTTPD_RESP_USE_STRLEN);
    
    free(json); // Libère le buffer de texte généré par cJSON
    return res;
}

/* -------------------------------------------------------------------------- */
/* POST /api/wifi (Set Config)                                               */
/* -------------------------------------------------------------------------- */
static esp_err_t wifi_config_post_handler(httpd_req_t *req)
{
    char buf[512];
    int len = httpd_req_recv(req, buf, sizeof(buf) - 1);
    if (len <= 0) {
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Requete vide");
    }
    ...
    if (cJSON_IsString(sta_ssid) && strlen(sta_ssid->valuestring) > 0) {
        strncpy(g_wifi_cfg.sta_ssid, sta_ssid->valuestring, sizeof(g_wifi_cfg.sta_ssid) - 1);
    }

    if (cJSON_IsString(sta_pass) && strcmp(sta_pass->valuestring, "undefined") != 0) {
        strncpy(g_wifi_cfg.sta_pass, sta_pass->valuestring, sizeof(g_wifi_cfg.sta_pass) - 1);
    }

    if (cJSON_IsString(ap_ssid) && strlen(ap_ssid->valuestring) > 0) {
        strncpy(g_wifi_cfg.ap_ssid, ap_ssid->valuestring, sizeof(g_wifi_cfg.ap_ssid) - 1);
    } else {
        ok = false;
    }
    ...
    if (cJSON_IsString(ap_pass) && strcmp(ap_pass->valuestring, "undefined") != 0) {
        ...
        if (lenp == 0) {
            ...
            strncpy(g_wifi_cfg.ap_pass, ap_pass->valuestring, sizeof(g_wifi_cfg.ap_pass) - 1);
        } else {
            ok = false;
        }
    }
    ...
    cJSON_Delete(root); // Libération systématique de la mémoire lue avant le retour réseau

    if (!ok) {
        httpd_resp_set_status(req, "400 Bad Request");
        return httpd_resp_sendstr(req, "{\"status\":\"error\",\"message\":\"Configuration invalide\"}");
    }
    ...
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_sendstr(req, "{\"status\":\"ok\",\"message\":\"Configuration sauvegardee\"}");
}
...
static esp_err_t wifi_scan_api_handler(httpd_req_t *req)
{
    ...
    wifi_ap_record_t *ap_records = malloc(sizeof(wifi_ap_record_t) * number);
    if (!ap_records) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM AP Records");
    }

    if (esp_wifi_scan_get_ap_records(&number, ap_records) != ESP_OK) {
        free(ap_records);
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Fetch records failed");
    }

    cJSON *root = cJSON_CreateArray();
    if (!root) {
        free(ap_records);
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM JSON Array");
    }

    for (int i = 0; i < number; i++) {
        cJSON *item = cJSON_CreateObject();
        if (!item) {
            // CORRECTION CRUCIALE : Nettoyage complet pour éviter la fuite de mémoire (OOM)
            cJSON_Delete(root);
            free(ap_records);
            return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM JSON Item");
        }
        
        char ssid_safe[33] = {0};
        strncpy(ssid_safe, (char *)ap_records[i].ssid, sizeof(ssid_safe) - 1);
    ...
    char *json_res = cJSON_PrintUnformatted(root);
    cJSON_Delete(root); // Libère l'arbre cJSON immédiatement
    free(ap_records);   // Libère le tableau temporaire de records

    if (!json_res) {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OOM Print Result");
    }

    httpd_resp_set_type(req, "application/json");
    esp_err_t res = httpd_resp_send(req, json_res, HTTPD_RESP_USE_STRLEN);
    
    free(json_res); // Libère le buffer de texte final
    return res;
}
...
static esp_err_t wifi_api_handler(httpd_req_t *req)
{
    ...
    // CORRECTION : Validation des handles netif avant requête (Évite Kernel Panic)
    void *sta_handle = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    void *ap_handle  = esp_netif_get_handle_from_ifkey("WIFI_AP_DEF");

    if (sta_handle) esp_netif_get_ip_info(sta_handle, &ip_sta);
    if (ap_handle)  esp_netif_get_ip_info(ap_handle, &ip_ap);
    ...
    snprintf(json, sizeof(json),
             "{\"ip_sta\":\"" IPSTR "\", \"ip_ap\":\"" IPSTR "\", \"rssi\":%d, "
             "\"auth\":\"%s\", \"mac\":\"%s\", \"chan\":%d, \"clients\":%u}",
             IP2STR(&ip_sta.ip), IP2STR(&ip_ap.ip),
             rssi, auth_mode, mac_str, channel, (unsigned int)client_count);
...
static esp_err_t api_wifi_connect(httpd_req_t *req)
{
    ...
    int len = httpd_req_recv(req, buf, sizeof(buf) - 1);
    if (len <= 0) {
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Corps vide");
    }
    ...
    if (cJSON_IsString(ssid) && cJSON_IsString(pass)) {
        wifi_manager_try_connect(ssid->valuestring, pass->valuestring);
        httpd_resp_set_type(req, "application/json");
        httpd_resp_sendstr(req, "{\"status\":\"pending\", \"message\":\"Essai lance\"}");
    } else {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "SSID/PASS manquants");
    }

    cJSON_Delete(root); // Libération propre de l'arbre parsé
    return ESP_OK;
}
...
esp_err_t ws_register_wifi_api(httpd_handle_t server)
{
    ...
    ESP_LOGI(TAG, "API WiFi completes enregistrees");
    return ESP_OK;
}
