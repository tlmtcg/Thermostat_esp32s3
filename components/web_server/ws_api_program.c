...
esp_err_t ws_register_program_api(httpd_handle_t server)
{
    ...
    ESP_LOGI(TAG, "Register: %s (POST)", uri_set.uri);

    err = httpd_register_uri_handler(server, &uri_set);
    ESP_LOGI(TAG, "Result /set -> %s", esp_err_to_name(err));

    // ---------------- RESET PROGRAM ----------------
    httpd_uri_t uri_reset = {
        .uri = "/api/program/reset",
        .method = HTTP_GET,
        .handler = reset_program_handler,
        .user_ctx = NULL};

    ESP_LOGI(TAG, "Register: %s (GET)", uri_reset.uri);

    err = httpd_register_uri_handler(server, &uri_reset);
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Program API registration FAILED");
        return err;
    }
