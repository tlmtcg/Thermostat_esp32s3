...
#include "cJSON.h"
#include "esp_heap_caps.h" // Pour l'allocation mémoire résiliente
#include <stdlib.h>
#include <string.h>
...
esp_err_t get_fb_list_handler(httpd_req_t *req)
{
    // Allocation résiliente (PSRAM avec repli RAM interne) pour le gros buffer de listing
    char *list_buf = heap_caps_malloc(4096, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (list_buf == NULL)
    {
        list_buf = malloc(4096);
    }
...
esp_err_t get_fb_read_handler(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK)
    {
        if (httpd_query_key_value(query, "file", filename, sizeof(filename)) == ESP_OK)
        {
            ...
            size_t buf_size = 4096;
            char *data = heap_caps_malloc(buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
            if (!data) data = malloc(buf_size);
            ...
            // CORRECTION : Libération et envoi d'erreur si le téléchargement FTP échoue
            free(data);
            return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "FTP Download Error");
        }
    }

    return httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "Fichier non trouve");
}
...
esp_err_t post_fb_save_handler(httpd_req_t *req)
{
    ...
    char *buf = heap_caps_malloc(total_len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!buf) buf = malloc(total_len + 1);
    ...
    cJSON *root = cJSON_Parse(buf);
    free(buf); // Libération du buffer de réception dès que possible après parsing
    ...
    if (cJSON_IsString(file_item) && cJSON_IsString(content_item))
    {
        ...
        if (freebox_ftp_upload(file_item->valuestring, content_item->valuestring, strlen(content_item->valuestring)) == ESP_OK)
        {
            httpd_resp_sendstr(req, "Upload OK");
            ret = ESP_OK;
        }
    ...
    {
        // CORRECTION : Réponse d'erreur si la structure JSON n'est pas conforme
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Missing JSON fields");
    }

    // CORRECTION CRUCIALE : Libération systématique de l'arbre cJSON, peu importe la validité des champs internes
    cJSON_Delete(root);
    return ret;
}
...
esp_err_t delete_fb_handler(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK)
    {
        if (httpd_query_key_value(query, "file", filename, sizeof(filename)) == ESP_OK)
        {
            ...
            if (freebox_ftp_delete(filename) == ESP_OK)
            {
                return httpd_resp_sendstr(req, "Fichier efface");
            }
            else
            {
                return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Erreur FTP DELE");
...
esp_err_t post_fb_rename_handler(httpd_req_t *req)
{
    ...
    {
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Taille invalide");
    }

    // CORRECTION : Allocation dynamique sécurisée au lieu d'une allocation sur la pile exposée aux crashs
    char *buf = malloc(total_len + 1);
    if (!buf)
    {
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "RAM Error");
    ...
    if (cJSON_IsString(old_n) && cJSON_IsString(new_n))
    {
        if (freebox_ftp_rename(old_n->valuestring, new_n->valuestring) == ESP_OK)
        {
            httpd_resp_sendstr(req, "OK");
            ret = ESP_OK;
        }
        ...
        {
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "FTP Rename Failed");
        }
    }
    else
    {
...
esp_err_t ws_register_freebox_api(httpd_handle_t server)
{
    ...
    return httpd_register_uri_handler(server, &uri_fb_del);
}
