...
esp_err_t i2c_manager_deinit(void)
{
     ...
     * IMPORTANT:
     *
     * Tous les devices I2C doivent avoir été supprimés
     * AVANT cet appel.
     *
     * Exemple:
     *
    ...
    esp_err_t err = i2c_del_master_bus(
        i2c_bus
    );
...
esp_err_t i2c_manager_scan(void)
{
    ...
    for (uint8_t addr = 0x08;
        ...
        if (err == ESP_OK) {
            ...
            ESP_LOGI(TAG,
                     "Device trouvé: 0x%02X",
                     addr);

            scan_result.addresses[
                scan_result.count++
            ] = addr;
        }
    }
    ...
    ESP_LOGI(TAG,
             "Scan terminé (%d devices)",
             (int)scan_result.count);
