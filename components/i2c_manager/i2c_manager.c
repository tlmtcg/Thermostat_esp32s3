...
#include "sdkconfig.h"
#include "time_utils.h"
#include "esp_heap_caps.h" // Ajouté pour le heap_caps
...
/* -------------------------------------------------------------------------- */
/* FREE JSON & SCAN (Nettoyage centralisé)                                    */
/* -------------------------------------------------------------------------- */
static void free_json(void)
{
    if (i2c_devices_json) {
...
esp_err_t i2c_manager_scan(void)
{
    ...
    // Nettoyage des scans précédents
    free_json();
    free_scan();

    // CORRECTION 1 : Allocation résiliente (PSRAM avec repli RAM interne)
    scan_result.addresses = heap_caps_malloc(128 * sizeof(uint8_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!scan_result.addresses) {
        scan_result.addresses = malloc(128 * sizeof(uint8_t));
    }
    ...
    // Initialisation de l'objet racine cJSON
    i2c_devices_json = cJSON_CreateObject();
    if (!i2c_devices_json) {
        goto fail_mem;
    }

    if (!cJSON_AddStringToObject(i2c_devices_json, "timestamp", scan_result.timestamp)) {
        goto fail_mem;
    }
    ...
    for (size_t i = 0; i < scan_result.count; i++) {
        cJSON *obj = cJSON_CreateObject();
        if (!obj) {
            goto fail_mem; // CORRECTION 2 : Si un sous-objet échoue, on traite l'erreur globale
        }
        cJSON_AddNumberToObject(obj, "address", scan_result.addresses[i]);
        cJSON_AddItemToArray(arr, obj);
    }
...
fail_mem:
    // CORRECTION 3 : Bloc d'évacuation en cas d'erreur de mémoire pour éviter tout leak
    ESP_LOGE(TAG, "Mémoire insuffisante durant la génération du JSON de scan");
    free_json();
    free_scan();
    return ESP_ERR_NO_MEM;
}
